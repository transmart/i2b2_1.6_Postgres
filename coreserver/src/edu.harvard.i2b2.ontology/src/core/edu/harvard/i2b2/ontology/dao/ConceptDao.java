...
// Lookup to get chlevel + 1 ---  dont allow synonyms so we only get one result back
	...
	//		}
		    ...
		    if(vocabType.getMatchStr().getStrategy().equals("exact")) {
		    	nameInfoSql = "select " + parameters  + " from " + metadataSchema+categoryResult.get(0).getTablename() + " where upper(c_name) = ? and c_fullname like '" + category +	"%'";    	
		    	compareName = value.toUpperCase();
		    	
		    }
		    
		    else if(vocabType.getMatchStr().getStrategy().equals("left")){
		    	nameInfoSql = "select " + parameters  + " from " + metadataSchema+categoryResult.get(0).getTablename() +" where upper(c_name) like ?  and c_fullname like '" + category +"%'";
		    	...
		    	if(!(value.contains(" "))){
		    		...
		    		nameInfoSql = nameInfoSql + parseMatchString(value)+ " and c_fullname like '" + category +"%'";;
		    		compareName = null;
		    	}
		    }
...
//		String sql = "select " + parameters + " from "+ metadataSchema+ tableName + " where m_exclusion_cd is null and c_fullname in (";
		...
		while (path.length() > 2) {
			if(path.endsWith("%")){
				...
				path = path + "%";
			inclusionSql = inclusionSql + " union all (select c_fullname from " + metadataSchema+ tableName  + " where m_applied_path = '" + path + "' and c_hlevel = 1 and m_exclusion_cd is null)";
...
//			queryResult = jt.query(sql, modMapper, path );
	...
	//		log.info("getChildren " + tableSql);
		...
		ParameterizedRowMapper<ModifierType> modMapper = getModMapper(new NodeType (modifierChildrenType), ofuscatedUserFlag);
	...
	public List findByFullname(final GetModifierInfoType modifierInfoType, ProjectType projectInfo, DBInfoType dbInfo) throws DataAccessException, I2B2Exception{
		...
		ParameterizedRowMapper<ModifierType> modMapper = getModMapper(new NodeType (modifierInfoType), ofuscatedUserFlag);
