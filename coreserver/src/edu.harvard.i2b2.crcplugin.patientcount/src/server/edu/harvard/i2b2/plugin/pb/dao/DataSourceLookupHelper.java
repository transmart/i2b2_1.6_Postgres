...
public class DataSourceLookupHelper {
	...
	protected final static Log log = LogFactory
			.getLog(DataSourceLookupHelper.class);
	...
	public DataSourceLookupDAO getLookupDAO() {
		return dsLookupDao;
	}
	...
	public DataSourceLookup matchDataSource(String hiveId, String projectId,
		...
		if (projectId == null) {
			throw new I2B2Exception(
					" Project id is null, could not perform datasource lookup for given domain["
							+ hiveId + "] and owner[" + ownerId + "]");
		}
		...
		// check if project id contains subprojects, if so then query by first
		// level subproject
		...
		dataSourceLookupList = dsLookupDao.getDbLookupByHiveProjectOwner(
				hiveId, filterProjectId, ownerId);
		...
		if (dataSourceLookupList.size() > 0) {
			...
			while ((projectLevel = projectId.lastIndexOf('/', projectLevel)) > 0) {
				...
				matchedDataSource = match(parentProjectId, ownerId,
						dataSourceLookupList);
			...
			if (matchedFlag) {
				...
				log.info("Could not match Project id=[" + projectId
						+ "] Trying with hive =" + hiveId);
				matchedDataSourceLookup = matchHiveOwner(hiveId, ownerId);
				return matchedDataSourceLookup;
			}
	...
	private DataSourceLookup matchHiveOwner(String hiveId, String ownerId)
			throws I2B2Exception {
		DataSourceLookup matchedDataSourceLookup = null;
		// check for hive and owner (userid or @)
		List<DataSourceLookup> dataSourceLookupList = dsLookupDao
				.getDbLookupByHiveOwner(hiveId, ownerId);
		if (dataSourceLookupList.size() == 0) {
			...
			for (DataSourceLookup dsLookup : dataSourceLookupList) {
				if (dsLookup.getOwnerId() != null
						&& dsLookup.getOwnerId().equalsIgnoreCase(ownerId)) {
					matchedDataSourceLookup = dsLookup;
					matchedFlag = true;
					break;
				} else if (dsLookup.getOwnerId() != null
						&& dsLookup.getOwnerId().equals("@")) {
					matchedDataSourceLookup = dsLookup;
				}
			}
			if (matchedFlag) {
				log.info("Located Datasource matching hive=[" + hiveId
						+ "] and owner=[" + ownerId + "]");
				...
				throw new I2B2Exception(
						"Could not locate Datasource matching hive=[" + hiveId
								+ "] and owner=[" + ownerId + " or @]");
			}
		}
		return matchedDataSourceLookup;
	}

	private DataSourceLookup match(String searchProjectId, String owner,
