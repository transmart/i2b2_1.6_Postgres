...
public class OracleDataSourceLookupDAO extends DataSourceLookupDAO  {
	...
	public List<DataSourceLookup> getDbLookupByHiveOwner(String domainId,String ownerId) { 
		String sql =  "select * from crc_db_lookup where c_domain_id = ? and c_project_path = ? and (c_owner_id = ? or c_owner_id ='@') order by c_project_path";
		String projectId = "@";
	...
	public List<DataSourceLookup> getDbLookupByHiveProjectOwner(String domainId, String projectId,
			String ownerId) {
		String sql = "select * from crc_db_lookup where c_domain_id = ? and c_project_path like  ? and (c_owner_id =? or c_owner_id = '@') order by c_project_path"; 
		List<DataSourceLookup> dataSourceLookupList = this.query(sql, new Object[]{domainId,projectId+"%",ownerId},new int[]{Types.VARCHAR,Types.VARCHAR,Types.VARCHAR},new mapper()  );
		return dataSourceLookupList;
	}
