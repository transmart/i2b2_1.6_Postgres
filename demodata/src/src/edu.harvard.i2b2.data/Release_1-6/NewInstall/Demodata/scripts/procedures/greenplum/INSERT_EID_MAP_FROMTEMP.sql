create or replace FUNCTION insert_eid_map_fromtemp(IN tempEidTableName VARCHAR, IN upload_id NUMERIC,
...
-- and t1.encounter_id_source = t2.encounter_id_source) ';
 ...
 execute 'delete  from ' || tempEidTableName ||  ' t1  
		  where (gp_segment_id, ctid) NOT IN (select gp_segment_id, max(ctid) from ' || tempEidTableName || ' t2 
		  group by gp_segment_id, encounter_map_id,encounter_map_id_source,encounter_id,encounter_id_source )';
