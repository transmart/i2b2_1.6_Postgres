create or replace FUNCTION  sync_clear_provider_table (in tempProviderTableName VARCHAR, in backupProviderTableName VARCHAR, in uploadId NUMERIC, OUT errorMsg VARCHAR ) 
AS $$$ DECLARE 
...
BEGIN 
	...
	execute 'DELETE FROM ' || tempProviderTableName || ' t1 
			 WHERE (gp_segment_id, ctid) NOT IN (
				SELECT gp_segment_id, max(ctid) 
				FROM ' || tempProviderTableName || ' t2
				GROUP BY gp_segment_id,  provider_path,provider_id)';				
