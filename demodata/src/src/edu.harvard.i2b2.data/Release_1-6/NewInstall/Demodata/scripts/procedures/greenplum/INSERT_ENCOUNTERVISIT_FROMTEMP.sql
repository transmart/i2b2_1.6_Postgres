create or replace function  insert_encountervisit_fromtemp (IN tempTableName VARCHAR, IN upload_id NUMERIC,
...
BEGIN 
	...
	execute  'DELETE FROM ' || tempTableName || ' t1 
			  WHERE (gp_segment_id, ctid) NOT IN (
					SELECT gp_segment_id, max(ctid) FROM ' || tempTableName || ' t2
					GROUP BY gp_segment_id, encounter_id,encounter_id_source)';
