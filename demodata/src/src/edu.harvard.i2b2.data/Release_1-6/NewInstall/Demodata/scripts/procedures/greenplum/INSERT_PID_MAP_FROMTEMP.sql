CREATE OR REPLACE FUNCTION insert_pid_map_fromtemp (IN tempPidTableName VARCHAR,  IN upload_id NUMERIC, 
 ...
 sql_stmt := ' SELECT distinct patient_id,patient_id_source from ' || tempPidTableName ||' ';
  ...
  execute 'delete  from ' || tempPidTableName ||  ' t1  
		   where (gp_segment_id, ctid) 
		    not in (select gp_segment_id, max(ctid) from  ' || tempPidTableName ||  ' 
			group  BY gp_segment_id,patient_map_id,patient_map_id_source,patient_id,patient_id_source)';
