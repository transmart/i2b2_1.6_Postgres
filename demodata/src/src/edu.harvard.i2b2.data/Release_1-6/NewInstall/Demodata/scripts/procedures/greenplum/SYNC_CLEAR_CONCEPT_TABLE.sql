create or replace function  sync_clear_concept_table (in tempConceptTableName VARCHAR, IN backupConceptTableName VARCHAR, IN uploadId NUMERIC, OUT errorMsg VARCHAR ) 
AS $$$ DECLARE
...
BEGIN 
	...
	execute 'DELETE FROM ' || tempConceptTableName || ' t1 
	         WHERE (gp_segment_id, ctid) NOT IN  
					   (SELECT  gp_segment_id, max(rowid) FROM ' || tempConceptTableName || ' t2
