...
BEGIN 
	...
	execute  'DELETE FROM ' || tempModifierTableName || ' t1 WHERE (gp_segment_id, ctid) NOT IN  
			   (SELECT  gp_segment_id, max(ctid) FROM ' || tempModifierTableName || ' t2
				GROUP BY gp_segment_id, modifier_path,modifier_cd)';
