create or replace function update_observation_fact (IN upload_temptable_name VARCHAR, IN upload_id NUMERIC, IN appendFlag NUMERIC,
...
execute 'DELETE FROM ' || upload_temptable_name ||'  
		 WHERE (gp_segment_id, ctid) not in (
			SELECT gp_segment_id, max(ctid) FROM ' || upload_temptable_name ||' 
			GROUP BY gp_segment_id, encounter_id,encounter_id_source,patient_id,patient_id_source, concept_cd,start_date,modifier_cd,provider_id,instance_num ORDER BY encounter_id,encounter_id_source)';
